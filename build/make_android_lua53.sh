if [ -n "$ANDROID_NDK" ]; then
    ...
    export NDK=~/Library/Android/android-ndk-r21b
fi
...
function build() {
    ...
    cmake -H. -B${BUILD_PATH} -DPBC=ON -DANDROID_ABI=${ABI} -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=${NDK}/build/cmake/android.toolchain.cmake -DANDROID_NATIVE_API_LEVEL=${API} -DANDROID_TOOLCHAIN=clang -DANDROID_TOOLCHAIN_NAME=${TOOLCHAIN_ANME}
